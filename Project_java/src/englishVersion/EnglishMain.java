...
public class EnglishMain {
	...
	public static void main(String[] args) throws Exception {
		...
		while(true) {
			...
			if (Integer.parseInt(choose) == 1) {
				KoreaMain korean = new KoreaMain();
				korean.korean();
			}
			...
			try {			
				if(Integer.parseInt(mainmenunumber) == 1) {
					while (Integer.parseInt(mainmenunumber) == 1) { 			// 1. 고객 관리
						...
						if (Integer.parseInt(firstmenunumber) == 1) { 		// 1-1 입력
							firstMenuInputCtData();	
						} else if (Integer.parseInt(firstmenunumber) == 2) {  // 1-2 삭제 
					...
					while (Integer.parseInt(mainmenunumber) == 2) {         // 2. 고객 정보 보기
						...
						if (Integer.parseInt(secondmenunumer) == 1) { 	   // 2-1 전체
							secondMenuOneFullListView();										
						} else if (Integer.parseInt(secondmenunumer) == 2) { // 2-2 분류 
							secondMenuTwoCategoriListSetStandard();					
						} else if (Integer.parseInt(secondmenunumer) == 3) { // 2-3 항목
							secondMenuThreeItemalizeList();					
						} else if (Integer.parseInt(secondmenunumer) == 4) break;
						  else {
							printError();
							break;
						}
	...
	public static void printMainMenu() {
		...
		System.out.println("3. English Program Exit");
		System.out.println("===================================================");
	...
	public static void firstMenuInputCtData() throws Exception {
		...
		// 고객 명수 만큼 배열 만들기 과정
		ct = new EnglishCustomer[Integer.parseInt(inputCtNumberOf)];
		for (i = 0; i < ct.length; i++) {
			ct[i] = new EnglishCustomer();
		}
		...
		while (Integer.parseInt(inputCtNumberOf) > i) {
			...
			System.out.printf("▶ [ID] %s [Location] %s \n  [Age] %d [Gender] %s \n  [Internet use Time(basis Month)] %d(h) \n", id, loc, age, gen, time);
			System.out.println("---------------------------------------------------");
	...
	public static void secondMenuTwoCategoriListSetStandard() throws Exception {
		...
		EnglishCustomer [] Default;
		EnglishCustomer [] Likely;
		EnglishCustomer [] Ideal;
		...
		Default = new EnglishCustomer[cnt_d];
		Likely = new EnglishCustomer[cnt_l];
		Ideal = new EnglishCustomer[cnt_i];
	...
	public static void secondMenuThreeItemalizeList() throws Exception{
		...
		while(true) {
			...
			if (secondMenuThreeCategori == 1) {
				...
				Arrays.sort(ct, new Comparator<EnglishCustomer>() { // 숫자로 시작할 경우...
					@Override
					public int compare(EnglishCustomer o1, EnglishCustomer o2) {
			...
			// 나이	
				...
				Arrays.sort(ct, new Comparator<EnglishCustomer>() {
					@Override
					public int compare(EnglishCustomer o1, EnglishCustomer o2) {
			...
			// 지역
				...
				EnglishCustomer [] longg = new EnglishCustomer[cnt_gg];
				EnglishCustomer [] longw = new EnglishCustomer[cnt_gw];
				EnglishCustomer [] longs = new EnglishCustomer[cnt_gs];
				EnglishCustomer [] loncc = new EnglishCustomer[cnt_cc];
				EnglishCustomer [] lonjj = new EnglishCustomer[cnt_jj];
				EnglishCustomer [] lonjl = new EnglishCustomer[cnt_jl];
